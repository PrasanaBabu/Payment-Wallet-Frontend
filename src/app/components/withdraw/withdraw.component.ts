...
export class WithdrawComponent {
  ...
  confirm() {
    this.confirmationService.confirm({
      message: 'Are you sure  you want to add ' + this.amountToWithdraw + ' to ID: ' + this.userId,
      header: 'Confirmation',
      icon: 'pi pi-exclamation-triangle',
      accept: () => {
        if (this.userId == undefined || this.amountToWithdraw == "") {
          this.msgs = [
            {
              severity: 'error',
              summary: 'Invalid',
  ...
  addFund() {
    ...
    console.log("Accept method clicked with amount = " + this.amountToWithdraw + " id = " + this.userId);
    this.transaction.amount = parseInt(this.amountToWithdraw);
